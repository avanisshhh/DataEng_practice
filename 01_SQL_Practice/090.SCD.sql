...
'''
For every UPDATED row (old version expired)
You insert a NEW row (new version)
Why INSERT outside MERGE in SCD Type 2?”
👉 “Because MERGE cannot perform UPDATE and INSERT for the same matched row. So we use OUTPUT to capture updated rows and insert new version records separately.”
'''

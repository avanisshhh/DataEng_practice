...
FROM Orders
--For each row → it picks previous row’s Qty

SELECT ProductId,InvoiceNum,OrderDate,City,Qty,
LAG(Qty,1,0) OVER(PARTITION BY CITY ORDER BY OrderDate )
FROM Orders
'''
Data is divided by City
Within each city → sorted by date
Takes previous Qty within same city
If no previous row → return 0 instead of NULL
'''

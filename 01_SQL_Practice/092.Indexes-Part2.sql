...
--A Non-Clustered Index is an index that helps SQL Server find data faster without changing the physical order of the table. 


CREATE INDEX Nic_deptcity ON EMPLOYEES4(Department,City)
--This creates a NON-CLUSTERED INDEX (by default)
...
SQL finds EmpName in index
Then goes to table → extra lookup ❌

👉 With INCLUDE:

Everything is already in index → no lookup ✅
'''

CREATE UNIQUE NONCLUSTERED INDEX Nic_Email ON EMPLOYEES4(Email)
'''
Creates a Non-Clustered Index + Enforces Uniqueness
Rules:
No duplicate emails allowed 
NULL behavior depends on DB (SQL Server allows one NULL)
'''
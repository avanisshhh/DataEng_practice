...
--ROW NUMBER: It is a window function that assigns a unique sequential integer to rows within a result set, based on the order specified in the OVER clause. 
SELECT EmpName, City, Department,Salary,
ROW_NUMBER() OVER(ORDER BY EmpName) RowNum
FROM EMPLOYEES
'''
What happens if 2 employees have the same EmpName?
👉 ROW_NUMBER() will still assign unique numbers, but:
⚠️ The order between those same names is NOT guaranteed (non-deterministic)
'''

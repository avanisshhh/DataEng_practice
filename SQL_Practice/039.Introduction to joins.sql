...
--join is used to combine rows from two or more tables based on a related column between them. In this case, we are joining the SalesTran and Products tables based on the ProductId column, which is common to both tables. The ON clause specifies the condition for the join, which is that the ProductId in the SalesTran table must match the ProductId in the Products table. This allows us to retrieve data from both tables in a single query, combining the relevant information based on the specified relationship.
